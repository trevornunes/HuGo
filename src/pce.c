...
char cart_name[PATH_MAX] = "/accounts/1000/shared/misc/roms/pce/";
...
InitPCE (char *name, char *backmemname)
{
  ...
  {
      fprintf(stderr,"InitPCE: error name, CD_emulation ...\n");
	  return -1;
  }

  if (CartLoad (name))
  {
    fprintf(stderr,"CartLoad: failed\n");
    return -1;
  }
...
RunPCE (void)
{
  fprintf(stderr,"RunPCE\n");

  if (!ResetPCE ())
    exe_go ();

  fprintf(stderr,"RunPCE: finished ...\n");
  return 1;
}
#endif
